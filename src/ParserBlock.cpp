...
std::unique_ptr<BlockAST> Parser::parseBlock() {
    ...
    while (!isFinishedBlock()) {
        ...
        if (statement != nullptr) {
            ...
            m_isValid = false;
            ErrorHandler::logError(currentLine, u8"Syntax Error: invalid block statement!");
    ...
    if (isToken(TokenType::PUNCTUATION, punctuation::BLOCK_CLOSE)) {
        ...
        if (m_blockCount < 0) {
            ...
            m_isValid = false;
            ErrorHandler::logError(currentLine, u8"Syntax Error: invalid closing block that was never opened!");
        }
    }
    ...
    if (m_blockCount < 0 || (isToken(TokenType::EOF_TOKEN) && m_blockCount != 0)) {
        if(m_blockCount != 0 && lastOpenBlock.size() > 1){
            ...
            ErrorHandler::logError(lastOpenBlock.back(), u8"ControlFlow Error: opened bracket must be closed!");
            lastOpenBlock.erase(lastOpenBlock.end());
        }
        m_isValid = false;
    }
